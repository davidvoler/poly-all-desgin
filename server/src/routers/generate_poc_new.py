...
async def get_video_course(body: VideoCourseId, school_user: SchoolUser = Depends(current_ai_school_user)):
    ...
    # Deferred import: video_module.py imports _load_video_course_owned from
    # this module, so importing it back at top level here would cycle.
    from routers.video_module import fetch_video_modules
    course.modules = await fetch_video_modules(course.course_id)
    return course
