...
// Auto-submit helper for question types without an explicit Check step
const MemoryAutoSubmit = ({ onSubmit }) => {
  React.useEffect(() => { onSubmit(); }, []);
  return null;
};

const QuizPage = ({ onExit, onFinish }) => {
  const questions = [
    { type: "translate",
      sentence: "こんにちは",
      translit: "konnichiwa",
  ...
  const initialSelected = (q) => q.type === "identify" ? [] : null;

  const [idx, setIdx] = React.useState(0);
  // Per-question state — preserved across navigation
  const [selectedByIdx, setSelectedByIdx] = React.useState(
    questions.map(initialSelected)
  );
  const [submittedByIdx, setSubmittedByIdx] = React.useState(
    questions.map(() => false)
  );
  ...
  const setSelected = (val) => {
    setSelectedByIdx(arr => arr.map((v, i) => i === idx ? val : v));
  };
  const setSubmitted = (val) => {
    setSubmittedByIdx(arr => arr.map((v, i) => i === idx ? val : v));
  };
  ...
  const canCheck = q.type === "translate" ? selected !== null
    ...
    : true;
  const allSubmitted = submittedByIdx.every(Boolean);
    ...
    <div className="page page-quiz">
      ...
      {q.type === "identify" && (
        <IdentifyWordsQuestion q={q} selected={selected} submitted={submitted}
          onToggle={i => setSelected(selected.includes(i) ? selected.filter(x => x !== i) : [...selected, i])} />
      )}
      {q.type === "memory" && <MemoryQuestion />}

      {/* Memory questions auto-mark as submitted so Finish can appear */}
      {q.type === "memory" && !submitted && (
        <MemoryAutoSubmit onSubmit={() => setSubmitted(true)} />
      )}

      {/* Primary action — Check Answer / Finish (when on last + everything done) */}
      {!submitted && q.type !== "memory" && (
        <PrimaryButton onClick={() => setSubmitted(true)} disabled={!canCheck}>
          Check Answer
        </PrimaryButton>
      )}
      {/* Next Lesson available on last page (even if some answers were skipped) */}
      {isLast && (
        <PrimaryButton onClick={onFinish || onExit}>Next Lesson</PrimaryButton>
      )}

      {/* Nav row — Back / Skip or Next */}
      <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
        <button
          ...
          style={{
            ...
            opacity: isFirst ? 0.4 : 1, cursor: isFirst ? "not-allowed" : "pointer",
          }}>
        ...
        <button
          ...
          style={{
            flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6,
            opacity: isLast ? 0.4 : 1, cursor: isLast ? "not-allowed" : "pointer",
          }}>
          {submitted ? "Next" : "Skip"}
