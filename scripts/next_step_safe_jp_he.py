...
async def gen_and_save_module(m:dict):
    ...
    os.makedirs(f"../data/content/ja_he/v1/module_{module_no}", exist_ok=True)
    if len(os.listdir(f"../data/content/ja_he/v1/module_{module_no}")) > 0:
        print(f"module {m.get('module')} already exists, skipping")
        return
    gen_m =  await gen_module(m)
    with open(f"../data/content/ja_he/v1/module_{module_no}/module.yaml", 'w') as f:
    ...
    for lesson in gen_m.get('lessons', []):
        ...
        with open(f"../data/content/ja_he/v1/module_{module_no}/lesson_{lesson_no}.txt", 'w') as f:
            ...
            for exercise in lesson.get('exercises', []):
                ...
                f.write(f"elements: {json.dumps(exercise.get('elements'))}\n")
                for o in options:
                    prefix = "[-]"
                    if o.get('correct'):
                        prefix = "[+]"
